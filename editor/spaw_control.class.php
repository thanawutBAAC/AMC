...
class SPAW_Wysiwyg {
  ...
  function getHtml()
  {
    ...
    if ($this->checkBrowser())
    {
      ...
      $buf .= '<td align="left" valign="top" width="90%">';
...
//  $buf.='setTimeout("SPAW_toggle_borders(\''.$n.'\',this[\''.$n.'_rEdit\'].document.body,null);",0);'."\n";
    ...
    else
    {
      // show simple text area
  	  $buf = '<textarea cols="200" rows="50" name="'.$n.'" style="width:'.$this->getWidth().'; height:'.$this->getHeight().'">'.htmlspecialchars($this->getValue()).'</textarea>';
    }
    return $buf;
  }
