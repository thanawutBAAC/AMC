<?php session_start();
	...
	$temp_report.="<tr> ";
	$temp_report.=" <td colspan='24' height='22'><font color='#0F7FCC'><b>&nbsp;&nbsp; มูลค่าธุรกิจแปรรูปผลผลิต (เชื่อมโยงธุรกิจการผลิตผลการตลาดโดยขบวนการสหกรณ์และองค์กรชุมชน) <font color='red'> มุมมอง ฝสข.</font></b></font></td> ";
	$temp_report.="</tr> ";
...
/*   เป้าหมายการจำหน่ายผลิตผล ของสกต */
	...
	WHILE($fetch_report = fetch_row($result_report)) { 
		...
		else    // แสดงธรรมดา
		{
			...
			{  $temp_report.="<tr>";  }
			$temp_report.=" <td width='120' nowrap>&nbsp;".trim($fetch_report[1])."  [".$current_amc[trim($fetch_report[1])]."]  </td>";   // สกต
	...
	$temp_report.="<br>";
	$temp_report.="<center><a href='excel_report181.php?div=".$div."&year=".$year."&month=".$month."'><img src='../images/page_excel.gif' border='0'> ข้อมูล Excel </a></center>";
	free_result($result_report);
    close();
