<?php session_start();
	...
	$temp_report.="<table width='800' class='gridtable' style='margin-top:25px; margin-left:5px; margin-right:5px;'> ";
...
//	$sql.=" AND report_year='".$year."' AND report_month='".$month."')AS Temp04 ";
	...
	WHILE($fetch_report = fetch_row($result_report)) { 
		...
		$temp_report.=" <td><a href='#' OnClick=' doCallAjax99(\"".trim($fetch_report[6])."\",\"".trim($fetch_report[1])."\")' > &nbsp;สกต.".trim($fetch_report[1])."</a>  [".$current_amc[$fetch_report[6]]."] </td>";
	...
	$temp_report.="<br>";
	$temp_report.="<center><a href='excel_report14.php?year=".$year."&month=".$month."&div=".$div."&province=".$province."&province_name=".$province_name."'><img src='../images/page_excel.gif' border='0'> ข้อมูล Excel </a></center>";
