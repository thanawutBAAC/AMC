<?php session_start();
	...
	header('Content-Disposition: attachment; filename="รายงานแปรรูปผลผลิตทั้งหมด มุมมอง ผลผลิต.xls"');
	header("Expires: 0");
...
<head>
<title>รายงานแปรรูปผลผลิตทั้งหมด สกต มุมมอง ผลผลิต</title>
...
/*   เป้าหมายการจำหน่ายผลิตผล ของสกต */
	...
	// สรุปรวมทั้งประเทศ
		...
		elseif(!is_null($fetch_report[0]) && is_null($fetch_report[1]) && is_null($fetch_report[2]) && is_null($fetch_report[3]))  
		{
			...
			$temp_report.=" <td colspan='2'>&nbsp; รวม ".trim($fetch_report[0])." </td>";
			$i = 0;
		...
		else    // แสดงธรรมดา
		{
			...
			{  $temp_report.="<tr>";  }
			$temp_report.=" <td width='80'>&nbsp;".trim($fetch_report[1])."</td>";   //  รายชื่อผลผลิต
			$temp_report.=" <td width='120'>&nbsp;".trim($fetch_report[2])."&nbsp;".trim($fetch_report[3])."  [".$current_amc[trim($fetch_report[3])]."] </td>";  // รายชื่อ สกต.
		}  // end if
	
		$temp_report.=" <td align='right'>".number_format($fetch_report[4],0,'','')."</td>";  // เป้าหมาย tris
		$temp_report.=" <td align='right'>".number_format($fetch_report[5],0,'','')."</td>";  // เป้าหมาย สกก 4  ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[6],0,'','')."</td>";  // เป้าหมาย สกก 4  พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[7],0,'','')."</td>";  // ราย 3.1
		$temp_report.=" <td align='right'>".number_format($fetch_report[8],0,'','')."</td>";  //  ตัน 3.1
		$temp_report.=" <td align='right'>".number_format($fetch_report[9],0,'','')."</td>";  //  พันบาท 3.1
		$temp_report.=" <td align='right'>".number_format($fetch_report[10],0,'','')."</td>";  //  องค์กร 3.2
		$temp_report.=" <td align='right'>".number_format($fetch_report[11],0,'','')."</td>";  //  ตัน 3.2 
		$temp_report.=" <td align='right'>".number_format($fetch_report[12],0,'','')."</td>";  // พันบาท 3.2 
		$temp_report.=" <td align='right'>".number_format($fetch_report[13],0,'','')."</td>";  //  ตัน 4
		$temp_report.=" <td align='right'>".number_format($fetch_report[14],0,'','')."</td>";  //  พัน 4
		$temp_report.=" <td align='right'>".number_format($fetch_report[15],0,'','')."</td>";  //  เป้าหมายสะสม สกก4 ประจำเดือน  5
		...
		$temp_report.=" <td align='right'>".number_format($temp_sum1,0,'','')."</td>";  //  ตัน 5
		$temp_report.=" <td align='right'>".number_format($temp_sum2,0,'','')."</td>";  //  พัน 5
		$temp_report.=" <td align='right'>".number_format($fetch_report[16],0,'','')."&nbsp;</td>";  //  6  เป้าหมายการจำหน่ายผลิตผล  ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[17],0,'','')."&nbsp;</td>";  //  6  เป้าหมายการจำหน่ายผลิตผล  พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[18],0,'','')."&nbsp;</td>";  //  7 จำหน่ายรวม ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[19],0,'','')."&nbsp;</td>";  //  7 จำหน่ายรวม พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[20],0,'','')."&nbsp;</td>";  //  7 จำหน่ายให้TABCO ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[21],0,'','')."&nbsp;</td>";  //  7 จำหน่ายให้TABCO พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[22],0,'','')."&nbsp;</td>";  //  8 มูลค่า ผลิตผลคงเหลือ
		$temp_report.=" <td align='right'>".number_format($fetch_report[23],0,'','')."&nbsp;</td>";  //  9 นายหน้าตัวแทน
		$temp_report.="</tr>";
