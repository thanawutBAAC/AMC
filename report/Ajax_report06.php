<?php session_start();
	...
	$temp_report.="   <td colspan='22' align='left'>&nbsp;<font color='#0F7FAF'><b> 3.1 รายงานมูลค่าธุรกิจรวบรวมผลิตผล (หน่วย: ตัน) </b></font></td> ";
	$temp_report.=" </tr> ";
 ...
 // สร้างข้อมูลส่วนเนื้อหา ตัน
	WHILE($fetch_report = fetch_row($result_report)) { 
		...
		$temp_report.=" <td><a href='#' OnClick=' doCallAjax99(\"".trim($fetch_report[7])."\",\"".trim($fetch_report[1])."\")' > &nbsp;สกต.".trim($fetch_report[1])."</a></td>";
