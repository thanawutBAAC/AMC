<?php session_start();
	...
	//สร้างข้อมูลหัวรายงาน
		$temp_report.="<table  width='1520'  class='gridtable' style='margin-top:15px; margin-left:5px; margin-right:5px;'> ";
...
//  ***********************   ตรวจสอบการส่งข้อมูลล่าสุดของ สกต นั้น  **************************
		...
		WHILE($fetch_amc = fetch_row($result_amc)) { 
			...
			$display_sql.=" ( userlogin.amccode='".trim($fetch_amc[0])."' AND report_month='".$month_report3[number_format($fetch_amc[1],0,'','')]."' ) OR";
			$display_sql2.=" ( userlogin.amccode='".trim($fetch_amc[0])."' AND TargetTris.target_month='".$month_report3[number_format($fetch_amc[1],0,'','')]."' ) OR";
...
$temp_report.="</table> ";
$temp_report.="<br>";
$temp_report.="<center><a href='excel_report02.php?year=".$year."&month=".$month."&div=".$div."&province=".$province."&province_name=".$province_name."'><img src='../images/page_excel.gif' border='0'> ข้อมูล Excel </a></center>";
