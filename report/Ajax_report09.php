...
// สร้างข้อมูลส่วนเนือ้หากลางตาราง
	$sql=" SELECT DISTINCT userlogin.br_code, BaseReportHeader.report_detail_code, ";
	$sql.=" BaseReportDetail.report_detail_name, ";
	$sql.=" Temp01.PlanCollectSell_Unit, Temp01.PlanCollectSell_Sum, Temp02.data9, ";
	$sql.=" Temp02.data10, Temp02.data11, Temp02.data12, Temp02.data13, Temp02.data14, ";
	$sql.=" Temp03.COUNT01 ";
	...
	$sql.=" PlanCollectSell.report_detail_code ,PlanCollectSell.amccode ";
	$sql.=" FROM PlanCollectSell ";
	$sql.=" WHERE PlanCollectSell.PlanCollectSell_year='".$year."' ";
	$sql.=" GROUP BY PlanCollectSell.report_detail_code,PlanCollectSell.amccode) AS Temp01 ";
	$sql.=" ON Temp01.report_detail_code = BaseReportHeader.report_detail_code ";
	$sql.=" AND Temp01.amccode=BaseReportHeader.amccode ";

	$sql.=" LEFT JOIN ( SELECT  SUM(data9)AS data9, SUM(data10)AS data10, SUM(data11)AS data11, ";
	...
	$sql.=" GROUP BY report_detail_code,amccode ) AS Temp02 ";
	$sql.=" ON Temp02.report_detail_code = BaseReportHeader.report_detail_code ";
	$sql.=" AND Temp02.amccode=BaseReportHeader.amccode ";

	$sql.=" LEFT JOIN( SELECT COUNT(DISTINCT BaseReportHeader.report_group_code)AS COUNT01, ";
	$sql.=" BaseReportHeader.report_detail_code, BaseReportHeader.amccode ";
	$sql.=" FROM BaseReportHeader ";
	$sql.=" WHERE BaseReportHeader.report_group_code='3' ";
	$sql.=" GROUP BY report_detail_code,amccode )AS Temp03 ";
	...
	$temp_report.=" <td align='left'>&nbsp; <a href='#' OnClick=' doCallAjax91(".$div.",".$year.",".$month.",".trim($fetch_report[1]).",\"".trim($fetch_report[2])."\") '>".trim($fetch_report[2])."(".$fetch_report[11].")</a></td> ";
