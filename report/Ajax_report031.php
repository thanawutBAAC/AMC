<?php session_start();
	...
	// สร้างหัวตารางรายงาน
	$temp_report.="<table  width='1420' class='gridtable' style='margin-top:15px; margin-left:5px; margin-right:5px;'>";
...
// สร้างข้อมูลส่วนเนือ้หากลางตาราง
	...
	WHILE($fetch_report = fetch_row($result_report)) { 
		...
		elseif( !is_null($fetch_report[0]) && !is_null($fetch_report[0]) && is_null($fetch_report[2]) && is_null($fetch_report[3]))   // สรุปรวมแต่ละสินค้า
		{
			...
			$temp_report.=" <td colspan='2'>&nbsp;<img id='pic".trim($fetch_report[0])."' src='../images/icon_plus.gif' style='cursor: hand'  onclick=showhide(page".trim($fetch_report[0]).",'pic".trim($fetch_report[0])."') > รวมสินค้า ".trim($fetch_report[1])." </td>";
			$i = 0;
		}
...
// <a href='#' OnClick=' doCallAjax99(\"".trim($fetch_report[8])."\",\"".trim($fetch_report[1])."\")' > &nbsp;สกต.".trim($fetch_report[1])."</a> 

		$temp_report.="  <td align='right'>".number_format($fetch_report[4])."&nbsp;</td> ";   // เป้าหมาย พันบาท ซื้อ
		$temp_report.="  <td align='right'>".number_format($fetch_report[13])."&nbsp;</td>";  // เป้าหมาย หน่วย ซื้อ
		$temp_report.="  <td align='right'>".number_format($fetch_report[5])."&nbsp;</td> ";  // จริง พันบาท
		$temp_report.="  <td align='right'>".number_format($fetch_report[10])."&nbsp;</td>";  // จริง หน่วย ซื้อ
		...
		$temp_report.="  <td align='right'>".number_format($fetch_report[12])."&nbsp;</td> ";  // จริง หน่วย tabco
		...
		$temp_report.="  <td align='right'>".number_format($fetch_report[14])."&nbsp;</td> ";  // เป้าหมาย หน่วย ขาย
		$temp_report.="  <td align='right'>".number_format($fetch_report[8])."&nbsp;</td> ";
		$temp_report.="  <td align='right'>".number_format($fetch_report[11])."&nbsp;</td> ";   // จริง หน่วย ขาย
		...
		//  ซ่อนข้อมูลฝ่ายในสินค้า เอาไว้ก่อน
	...
	$temp_report.="<br>";
	$temp_report.="<center><a href='excel_report031.php?div=".$div."&year=".$year."&month=".$month."&province=".$province."&province_name=".$province_name." '><img src='../images/page_excel.gif' border='0'> ข้อมูล Excel </a></center>";
	free_result($result_report);
    close();
