<?php session_start();
	...
	header('Content-Disposition: attachment; filename="รายงานรวบรวมผลิตผลใช้ติดตามเป้าหมาย .xls"');
	header("Expires: 0");
...
<head>
<title>รายงานรวบรวมผลิตผลใช้ติดตามเป้าหมาย</title>
...
<?
	...
	// สร้างหัวตารางรายงาน
	$temp_report.="<table width='1920' border='1' style='margin-top:15px; margin-left:5px; margin-right:5px;'> ";
	...
	WHILE($fetch_report = fetch_row($result_report)) { 
	 ...
	 //  ตรวจสอบว่ามีการสรุปรวม  ลักษณะราย สกต ที่ไม่ต้องการ ก็ไม่ต้องให้แสดง
		...
		else    // แสดงธรรมดา
		{
			...
			{  $temp_report.="<tr>";  }
			$temp_report.=" <td width='80'>&nbsp;".trim($fetch_report[1])."</td>";   // ผลผลิตย่อย
			$temp_report.=" <td width='120'>&nbsp;".trim($fetch_report[2])."&nbsp;".trim($fetch_report[3])."</td>";  // รายชื่อ ฝ่าย / สกต.
		}  // end if
		...
		if( number_format($fetch_report[23])==0){	 // เก็บข้อมูล เทียบ % ชั่วคราว
			$temp_sum3 = 100;
		}else{
