<?php session_start();
	...
	// เป้าสะสมเดือน สกก 4 
	$month_skk4 = array("1"=>'PlanCollectBuy_Jan',"2"=>'PlanCollectBuy_Feb',"3"=>'PlanCollectBuy_Mar',"4"=>'PlanCollectBuy_Apr',"5"=>'PlanCollectBuy_May',"6"=>'PlanCollectBuy_Jun',"7"=>'PlanCollectBuy_Jul',"8"=>'PlanCollectBuy_Aug',"9"=>'PlanCollectBuy_Sep',"10"=>'PlanCollectBuy_Oct',"11"=>'PlanCollectBuy_Nov',"12"=>'PlanCollectBuy_Dec');
	...
	// แสดงรายงานแปรรูปผลิตผล โดยเป็นมุมมอง ฝสข. 1-9 
	$year1 = $year+1;	

	$temp_report = "";  // ผลลัพธ์ที่จะต้องคืนค่า
	// หัวรายงาน
	$temp_report.="<br><br><strong><center>สหกรณ์การเกษตรเพื่อการตลาดลูกค้า ธ.ก.ส. จำกัด </center>";
	$temp_report.="<center> มูลค่าธุรกิจแปรรูปผลิตผล (เชื่อมโยงธุรกิจการผลิตการตลาดโดยขบวนการสหกรณ์และองค์กรชุมชน)  </center>";
	...
	// สร้างหัวตารางรายงาน
	$temp_report.="<table width='1950' class='gridtable' style='margin-top:15px; margin-left:5px; margin-right:5px;'> ";
	$temp_report.="<tr> ";
	$temp_report.=" <td colspan='24' height='22'><font color='#0F7FCC'><b>&nbsp;&nbsp; มูลค่าธุรกิจแปรรูปผลิตผล (เชื่อมโยงธุรกิจการผลิตผลการตลาดโดยขบวนการสหกรณ์และองค์กรชุมชน) <font color='red'> มุมมองผลผลิต </font></b></font></td> ";
	$temp_report.="</tr> ";
	$temp_report.="<tr bgcolor='#33CCCC'> ";
	$temp_report.=" <td rowspan='4' colspan='2' align='center' width='200'>ฝสข/จำนวน<br> สาขา/ผลิตผล <br> ข้อมูลเดือน  </td> ";
	$temp_report.=" <td rowspan='2' align='center'>(1) เป้าหมาย<br>ตามบันทึก<br>ข้อตกลง</td> ";
	$temp_report.=" <td colspan='2' rowspan='2' align='center'> (2) เป้าหมาย <br>การแปรรูป <br>ผลิตผล ของ สกต.</td> ";
	$temp_report.=" <td colspan='6'>&nbsp;&nbsp; 3. ผลการแปรรูประหว่างปี (ตัวชี้วัดของ Tris / บันทึกข้อตกลง)</td> ";
	...
	$temp_report.=" <td colspan='3' align='center'> (3.1) สกต.แปรรูป <br> ผลิตผล/ผลิตภัณฑ์ <br>จากสมาชิกและเกษตรกรทั่วไป</td> ";
	$temp_report.=" <td colspan='3' align='center'>(3.2) สนจ. สนับสนุน <br> องค์กรชุมชน แปรรูป <br> ผลิตผล/ผลิตภัณฑ์ผ่าน   สกต.</td> ";
	...
	$temp_report.=" <td colspan='3' align='center'> ผลการแปรรูป ถึง 31/03/".$year1."</td> ";
	$temp_report.=" <td colspan='3' align='center'> ผลการสนับสนุนแปรรูป</td> ";
	...
	$temp_report.=" <td width='80' align='center'> (ตัน) </td> ";
	$temp_report.=" <td width='80' align='center'> (ตัน) </td> ";
	$temp_report.=" <td width='80' align='center'> (พันบาท) </td> ";
	$temp_report.=" <td width='80' align='center'> ราย </td> ";
	$temp_report.=" <td width='80' align='center'> ตัน </td> ";
	$temp_report.=" <td width='80' align='center'> พันบาท </td> ";
	$temp_report.=" <td width='80' align='center'> องค์กร </td> ";
	$temp_report.=" <td width='80' align='center'> ตัน </td> ";
	$temp_report.=" <td width='80' align='center'> พันบาท </td> ";
	$temp_report.=" <td width='80' align='center'> ตัน </td> ";
	$temp_report.=" <td width='80' align='center'> พันบาท </td> ";
...
/* เป้าหมายการแปรรูป ผลิตผล ของ สกต. */
	$sql.=" LEFT JOIN (   ";
...
/* ผลการแปรรูประหว่างปี ตามบันทึกข้อตกลง  3  */
	$sql.=" LEFT JOIN (   ";
...
/*   เป้าหมายการจำหน่ายผลิตผล ของสกต */
	...
	WHILE($fetch_report = fetch_row($result_report)) { 
	 ...
	 //  ตรวจสอบว่ามีการสรุปรวม  ลักษณะราย สกต. กับ ผลผลิต ที่ไม่ต้องการ ก็ไม่ต้องให้แสดง
	...
	// สรุปรวมทั้งประเทศ
		...
		else    // แสดงธรรมดา
		{
			...
			{  $temp_report.="<tr>";  }
			$temp_report.=" <td width='250' nowrap>&nbsp;".trim($fetch_report[1])."</td>";   //  รายชื่อผลผลิต
			$temp_report.=" <td width='150' nowrap>&nbsp;".trim($fetch_report[2])."&nbsp;".trim($fetch_report[3])."  [".$current_amc[trim($fetch_report[3])]."] </td>";  // รายชื่อ สกต.
		}  // end if
	
		$temp_report.=" <td align='right'>".number_format($fetch_report[4])."&nbsp;</td>";  // เป้าหมาย tris
		$temp_report.=" <td align='right'>".number_format($fetch_report[5])."&nbsp;</td>";  // เป้าหมาย สกก 4  ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[6])."&nbsp;</td>";  // เป้าหมาย สกก 4  พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[7])."&nbsp;</td>";  // ราย 3.1
		$temp_report.=" <td align='right'>".number_format($fetch_report[8])."&nbsp;</td>";  //  ตัน 3.1
		$temp_report.=" <td align='right'>".number_format($fetch_report[9])."&nbsp;</td>";  //  พันบาท 3.1
		$temp_report.=" <td align='right'>".number_format($fetch_report[10])."&nbsp;</td>";  //  องค์กร 3.2
		$temp_report.=" <td align='right'>".number_format($fetch_report[11])."&nbsp;</td>";  //  ตัน 3.2 
		$temp_report.=" <td align='right'>".number_format($fetch_report[12])."&nbsp;</td>";  // พันบาท 3.2 
		$temp_report.=" <td align='right'>".number_format($fetch_report[13])."&nbsp;</td>";  //  ตัน 4
		$temp_report.=" <td align='right'>".number_format($fetch_report[14])."&nbsp;</td>";  //  พัน 4
		$temp_report.=" <td align='right'>".number_format($fetch_report[15])."&nbsp;</td>";  //  เป้าหมายสะสม สกก4 ประจำเดือน  5
		...
		$temp_report.=" <td align='right'>".number_format($fetch_report[16])."&nbsp;</td>";  //  6  เป้าหมายการจำหน่ายผลิตผล  ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[17])."&nbsp;</td>";  //  6  เป้าหมายการจำหน่ายผลิตผล  พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[18])."&nbsp;</td>";  //  7 จำหน่ายรวม ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[19])."&nbsp;</td>";  //  7 จำหน่ายรวม พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[20])."&nbsp;</td>";  //  7 จำหน่ายให้TABCO ตัน
		$temp_report.=" <td align='right'>".number_format($fetch_report[21])."&nbsp;</td>";  //  7 จำหน่ายให้TABCO พันบาท
		$temp_report.=" <td align='right'>".number_format($fetch_report[22])."&nbsp;</td>";  //  8 มูลค่า ผลิตผลคงเหลือ
		$temp_report.=" <td align='right'>".number_format($fetch_report[23])."&nbsp;</td>";  //  9 นายหน้าตัวแทน
		$temp_report.="</tr>";
	...
	$temp_report.="<br>";
	$temp_report.="<center><a href='excel_report182.php?div=".$div."&year=".$year."&month=".$month."'><img src='../images/page_excel.gif' border='0'> ข้อมูล Excel </a></center>";
	free_result($result_report);
    close();
