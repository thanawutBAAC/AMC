...
function doCallAjax() {  //  ข้อมูลธุรกิจรวบรวมผลิตผล  หมวดที่ 3
	HttPRequest = false; 
	...
	var url = 'Ajax_report04.php?year='+year+'&month='+month+'&div='+div+'&province='+province+'&province_name='+province_name;    // ดึงข้อมูลรายงาน
...
function doCallAjax5(div,year,month) {  //  แสดงข้อมูลรายงานรายละเอียด หมวด พันบาท
	HttPRequest = false; 
	if (window.XMLHttpRequest) { // Mozilla, Safari,... 
		HttPRequest = new XMLHttpRequest();
		if (HttPRequest.overrideMimeType) { 
	...
	var province = province_list.value;  // สกต จังหวัด
	var province_name = province_list.options[province_list.selectedIndex].text;  // ข้อมูล สกต.จังหวัดที่เลือก
	var url = 'Ajax_report05.php?div='+div+'&year='+year+'&month='+month+'&province='+province+'&province_name='+province_name;    // ดึงข้อมูลรายงาน
	document.getElementById("result_search").innerHTML = "Now is Loading... ";        // page loading
	document.getElementById("ajax_loading").style.display = '';
	HttPRequest.open('get',url,true); 
	HttPRequest.setRequestHeader('Content-Type', 'text/html; charset:windows-874');
...
function doCallAjax6(div,year,month) {  //  แสดงข้อมูลรายงานรายละเอียด หมวด ตัน
	HttPRequest = false; 
	if (window.XMLHttpRequest) { // Mozilla, Safari,... 
		HttPRequest = new XMLHttpRequest();
		if (HttPRequest.overrideMimeType) { 
	...
	var province = province_list.value;  // สกต จังหวัด
	var province_name = province_list.options[province_list.selectedIndex].text;  // ข้อมูล สกต.จังหวัดที่เลือก
	var url = 'Ajax_report06.php?div='+div+'&year='+year+'&month='+month+'&province='+province+'&province_name='+province_name;    // ดึงข้อมูลรายงาน
	document.getElementById("result_search").innerHTML = "Now is Loading... ";        // page loading
	document.getElementById("ajax_loading").style.display = '';
	HttPRequest.open('get',url,true); 
	HttPRequest.setRequestHeader('Content-Type', 'text/html; charset:windows-874');
   ...
   HttPRequest.send(null);
} // end function doCallAjax6
function doCallAjax41(div,year,month,report_detail_code,report_detail_name) {  //  แสดงข้อมูลรายงานรายละเอียด หมวด พันบาท
	HttPRequest = false; 
	if (window.XMLHttpRequest) { // Mozilla, Safari,... 
		HttPRequest = new XMLHttpRequest();
		if (HttPRequest.overrideMimeType) { 
	...
	var province = province_list.value;  // สกต จังหวัด
	var province_name = province_list.options[province_list.selectedIndex].text;  // ข้อมูล สกต.จังหวัดที่เลือก
	var url = 'Ajax_report041.php?div='+div+'&year='+year+'&month='+month+'&province='+province+'&province_name='+province_name+'&report_detail_code='+report_detail_code+'&report_detail_name='+report_detail_name;    // ดึงข้อมูลรายงาน
	document.getElementById("result_search").innerHTML = "Now is Loading... ";        // page loading
	document.getElementById("ajax_loading").style.display = '';
	HttPRequest.open('get',url,true); 
	HttPRequest.setRequestHeader('Content-Type', 'text/html; charset:windows-874');
   ...
   HttPRequest.send(null);
} // end function doCallAjax41
function doCallAjax42(div,year,month,report_detail_code,report_detail_name) {  //  แสดงข้อมูลรายงานรายละเอียด หมวด ตัน
	HttPRequest = false; 
	if (window.XMLHttpRequest) { // Mozilla, Safari,... 
		HttPRequest = new XMLHttpRequest();
		if (HttPRequest.overrideMimeType) { 
	...
	var province = province_list.value;  // สกต จังหวัด
	var province_name = province_list.options[province_list.selectedIndex].text;  // ข้อมูล สกต.จังหวัดที่เลือก
	var url = 'Ajax_report042.php?div='+div+'&year='+year+'&month='+month+'&province='+province+'&province_name='+province_name+'&report_detail_code='+report_detail_code+'&report_detail_name='+report_detail_name;    // ดึงข้อมูลรายงาน
	document.getElementById("result_search").innerHTML = "Now is Loading... ";        // page loading
	document.getElementById("ajax_loading").style.display = '';
	HttPRequest.open('get',url,true); 
	HttPRequest.setRequestHeader('Content-Type', 'text/html; charset:windows-874');
...
<?php  // แสดงข้อมูลหัวรายงาน
	$report_header ="4. รายงานมูลค่าธุรกิจรวบรวมผลผลิต (มุมมองผลิตผล) ";
	$report_type ="1";
