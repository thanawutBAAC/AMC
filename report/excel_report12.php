<?php session_start();
	...
	header('Content-Disposition: attachment; filename="มูลค่าธุรกิจให้บริการและส่งเสริมการเกษตร.xls"');
	header("Expires: 0");
...
<head>
<title> มูลค่าธุรกิจให้บริการและส่งเสริมการเกษตร </title>
...
<?
	...
	$temp_report.="<table width='580' border='1' style='margin-top:25px; margin-left:5px; margin-right:5px;'> ";
	...
	$temp_report.="</tr><tr bgcolor='#FF99CC'>";
...
//  **************************  สิ้นสุดการตรวจสอบ   ****************************************
	...
	WHILE($fetch_report = fetch_row($result_report)) { 
		...
		$temp_report.=" <td>&nbsp;สกต.".trim($fetch_report[1])." [".$current_amc[$fetch_report[4]]."] </td>";
