<?php session_start();
	...
	// สร้างหัวตารางรายงาน
	$temp_report.="<table width='2200' class='gridtable' style='margin-top:15px; margin-left:5px; margin-right:5px;'> ";
	$temp_report.="<tr> ";
	$temp_report.=" <td colspan='24' height='22'><font color='#0F7FCC'><b>&nbsp;&nbsp; มูลค่าธุรกิจรวบรวมผลิตผล (เชื่อมโยงธุรกิจการผลิตผลการตลาดโดยขบวนการสหกรณ์และองค์กรชุมชน) <font color='red'> มุมมองผลผลิต </font></b></font></td> ";
	$temp_report.="</tr> ";
...
/*   เป้าหมายการจำหน่ายผลิตผล ของสกต */
	...
	// สรุปรวมทั้งประเทศ
		...
		else    // แสดงธรรมดา
		{
			...
			{  $temp_report.="<tr>";  }
			$temp_report.=" <td width='250'>&nbsp;".trim($fetch_report[1])."</td>";   //  รายชื่อผลผลิต
			$temp_report.=" <td width='150'>&nbsp;".trim($fetch_report[2])."&nbsp;".trim($fetch_report[3])."  [".$current_amc[trim($fetch_report[3])]."] </td>";  // รายชื่อ สกต.
		}  // end if
	...
	$temp_report.="<br>";
	$temp_report.="<center><a href='excel_report152.php?div=".$div."&year=".$year."&month=".$month."'><img src='../images/page_excel.gif' border='0'> ข้อมูล Excel </a></center>";
	free_result($result_report);
    close();
