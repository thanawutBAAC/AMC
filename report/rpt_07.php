...
function doCallAjax() {  //  แสดงข้อมูลรายงานทั่วไป
	...
	var url = 'Ajax_report07.php?year='+year+'&month='+month+'&div='+div+'&province='+province+'&province_name='+province_name;    // ดึงข้อมูลรายงาน
...
function doCallAjax71(div,year,month,report_detail_code,report_detail_name) {  //  แสดงข้อมูลรายงานรายละเอียด หมวด ตัน
	HttPRequest = false; 
	if (window.XMLHttpRequest) { // Mozilla, Safari,... 
		HttPRequest = new XMLHttpRequest();
		if (HttPRequest.overrideMimeType) { 
	...
	var province = province_list.value;  // สกต จังหวัด
	var province_name = province_list.options[province_list.selectedIndex].text;  // ข้อมูล สกต.จังหวัดที่เลือก
	var url = 'Ajax_report071.php?div='+div+'&year='+year+'&month='+month+'&province='+province+'&province_name='+province_name+'&report_detail_code='+report_detail_code+'&report_detail_name='+report_detail_name;    // ดึงข้อมูลรายงาน
	document.getElementById("result_search").innerHTML = "Now is Loading... ";        // page loading
	document.getElementById("ajax_loading").style.display = '';
	HttPRequest.open('get',url,true); 
	HttPRequest.setRequestHeader('Content-Type', 'text/html; charset:windows-874');
...
<?php  // แสดงข้อมูลหัวรายงาน
	$report_header ="7. มูลค่าธุรกิจรวบรวมผลผลิต ธุรกิจรวบรวม+จำหน่าย ตัวชี้วัดของ Tris (มุมมองผลิตผล) ";
	$report_type ="1";
