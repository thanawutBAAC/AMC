	...
	for($i=0;$i<=($row-1);$i++)
			{
				...
				if($amccode[$i]!="" && $assetname[$i]=="0" && $assetcategory[$i]!="" ) //update ข้อมูลเดิม
					{
						$sql = "UPDATE AssetDetail SET Assetsize='$assetsize[$i]', AssetAmount='$assetamount[$i]', AssetValues='$assetvalues[$i]', Assetapplying='".$_GET['assetapplying'.$i]."', AssetRemark='$assetremark[$i]' ,Assettypeground='$assettypeground[$i]'";
				...
				if($amccode[$i]=="" && $assetname[$i]=="0" && $assetcategory[$i]!="" ) //update ข้อมูลเดิม
					{
							if($assetsize[$i]!="" || $assetamount[$i]!="" || $assetvalues[$i]!="" || $assetremark[$i]!="")
									{
											$sql="INSERT INTO AssetDetail  (AMCCode,AssetProvince, AssetType, AssetCategory, AssetSize, AssetAmount, AssetValues, AssetApplying, AssetRemark,AssetTypeGround) ";
											$sql.=" Values ('$amc','$province','$assettype[$i]', '$assetcategory[$i]', '$assetsize[$i]', '$assetamount[$i]', '$assetvalues[$i]', '".$_GET['assetapplying'.$i]."', '$assetremark[$i]','$assettypeground[$i]') ";
											mssql_query($sql);
