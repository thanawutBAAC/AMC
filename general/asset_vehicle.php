...
<!--
	...
	function assetapplyingDisable() {
		for(i=0;i<=(document.form1.length-1);i++) {
			if((document.forms[0].elements[i].name.substring(0,11)=="assetamount")&&(trim(document.forms[0].elements[i].value)=="")) {
				var y1 = i*1+1;
				var y2 = i*1+2;
				var y3 = i*1+3;
				if((trim(document.forms[0].elements[y1].value)=="")) {
					document.forms[0].elements[y2].disabled=true;
	...
	function chkItem(objA, objV, objP) {
		//alert("objS="+trim(objS.value)+"//");
		if ((trim(objA.value) != "")||(trim(objV.value) != "")) {
  ...
  <tr>
    <td align="center" valign="top"><form name="form1" method="post" action="asset_insetinto.php">
        <table width="80%" border="0" cellpadding="0" cellspacing="0">
          <tr> 
            <td align="center" valign="top"><table style="margin: 7px 0px 0px 0px" cellSpacing="1" cellPadding="0" width="720" border="0" class=font1 bgcolor=white>
                ...
                <tr bgcolor="#F0F0F0" align=center class=font1 style="color:#666666;"> 
                  <td height="20" align=left>&nbsp;&nbsp;&nbsp; 
					...
					?>
                    <b> </b> </td>
                  <td height="20"><input name="assetamount[]" type="text" class="AssetSize" id ="assetamount<?=$a?>"value="<?echo $rowall[5]?>" style="text-align: center;" onchange="chkItem(this,document.getElementById('assetvalue<?=$a?>'),document.forms[0].assetapplying<?=$i?>);" onkeypress=check_number();></td>
                  <td style="color:#FF0080;border:1px solid #F0F0F0;"> 
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="font1">
                      <tr> 
                        <td width="80%"><input name="assetvalues[]2" type="text" class="AssetSize" id="assetvalues[]" value="<?echo $rowall[6]?>" style="text-align: right;" onChange="chkItem(document.getElementById('assetamount<?=$a?>'),this,document.forms[0].assetapplying<?=$i?>);" onkeypress=check_number();></td>
                  ...
                  <td height="20"> 
                    ...
                    มี <? echo $green;?>
                    <input type="radio" name="assetapplying<?=$i?>"  value="0" <? if($applying=="0"){echo "checked";}?>> 
                    <? if($applying=="0"){echo"<span class='txtred'>";}?>
                    <? if($applying=="0"){$red="</span>";}?>
                    ไม่มี<?echo $red;?></td>
                  <td height="20" style="color:#FF0080;border:1px solid #F0F0F0;"><input name="assetremark[]2" type="text" class="AssetRemark"  id="assetremark[]2" value="<?echo $rowall[8];?>"></td>
                </tr>
                <?php
						$i++;			
				...
				?>
                    ...
                    </b> <input name="assetname[]" type="text" class="AssetType2" id="assetname<?=$a?>"></td>
                  <td height="20"> <input name="assetamount[]" type="text" class="Assetsize" id="assetamount<?=$a?>" style="text-align: center;" onchange="chkItem(this,document.forms[0].assetvalues<?=$a?>,document.forms[0].assetapplying<?=$i?>);"></td>
                  <td style="color:#FF0080;border:1px solid #F0F0F0;"><input name="assetvalues[]" type="text" class="Assetsize" id="assetvalues<?=$a;?>" style="text-align: right;" onchange="chkItem(document.forms[0].assetamount<?=$a?>,this,document.forms[0].assetapplying<?=$i?>);"></td>
				...
				?>
                <tr bgcolor="#F0F0F0" align=center class=font1 style="color:#666666;"> 
                  <td colspan="6" align=left class="boxleft10" height="7"> 
