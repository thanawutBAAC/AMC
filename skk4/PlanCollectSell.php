<?php	session_start();
	...
	$sql = " SELECT PlanCollectSell_year FROM PlanCollectSell ";
	$sql.=" WHERE amccode='".$code_online."' ";
	$sql.=" GROUP BY PlanCollectSell_year ";
	$sql.=" ORDER BY PlanCollectSell_year ";
...
</span>&nbsp;<a href="PlanCollectSell_detail.php?click=add" alt=" เพิ่มข้อมูล ">เพิ่มข้อมูล</a>
</div>
<table width="300" align="center"  class="gridtable" style="margin-top:5px;">
	...
	?>
			...
			<td align="center">
				<a href="PlanCollectSell_detail.php?click=edit&year=<?=$fetch_plan[0]; ?>" target="right" alt=" แก้ไขข้อมูล " style='cursor: hand'>
				...
				<a href="PlanCollectSell_sql.php?click=del&year=<?=$fetch_plan[0]; ?>" target="right" alt="ลบข้อมูล" style='cursor: hand' onclick=" return confirm_delete('<?=$fetch_plan[0];?>') " >
