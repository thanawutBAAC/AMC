<?php session_start();
	...
	$sql = " SELECT PlanExpenses_year FROM PlanExpenses ";
	$sql.=" WHERE amccode='".$code_online."' ";
	$sql.=" GROUP BY PlanExpenses_year ";
	$sql.=" ORDER BY PlanExpenses_year ";
...
</span>&nbsp;<a href="PlanExpenses_detail.php?click=add" alt=" เพิ่มข้อมูล ">เพิ่มข้อมูล</a>
</div>
<table width="300" align="center"  class="gridtable" style="margin-top:5px;">
	...
	?>
			...
			<td align="center"><?=trim($fetch_plan[0]); ?></td>
			<td align="center">
				<a href="PlanExpenses_detail.php?click=edit&year=<?=trim($fetch_plan[0]); ?>" target="right" alt=" แก้ไขข้อมูล " style='cursor: hand'>
				...
				<a href="PlanExpenses_sql.php?click=del&year=<?=trim($fetch_plan[0]); ?>" target="right" alt="ลบข้อมูล" style='cursor: hand' onclick=" return confirm_delete('<?=trim($fetch_plan[0]);?>') " >
