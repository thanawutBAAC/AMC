<?php session_start();
	...
	$sql = " SELECT PlanCollectBuy_year FROM PlanCollectBuy ";
	$sql.=" WHERE amccode='".$code_online."' ";
	$sql.=" GROUP BY PlanCollectBuy_year ";
	$sql.=" ORDER BY PlanCollectBuy_year ";
...
</span>&nbsp;<a href="PlanCollectBuy_detail.php?click=add" alt=" เพิ่มข้อมูล ">เพิ่มข้อมูล</a>
</div>
<table width="300" align="center"  class="gridtable" style="margin-top:5px;">
	...
	?>
			...
			<td align="center">
				<a href="PlanCollectBuy_detail.php?click=edit&year=<?=$fetch_plan[0]; ?>" target="right" alt=" แก้ไขข้อมูล " style='cursor: hand'>
				...
				<a href="PlanCollectBuy_sql.php?click=del&year=<?=$fetch_plan[0]; ?>" target="right" alt="ลบข้อมูล" style='cursor: hand' onclick=" return confirm_delete('<?=$fetch_plan[0];?>') " >
